...
def create_silver_layer() -> None:
    try:
        ...
        features_lf = features_lf.rename({col: col.strip().lower().replace(" ", "_") for col in features_lf.collect_schema().names()})
        stores_lf = stores_lf.rename({col: col.strip().lower().replace(" ", "_") for col in stores_lf.collect_schema().names()})

        # Deduplicate overlapping sales records
        logger.info("Deduplicating overlapping Date/Store/Dept records", dataset="sales")
        sales_lf = sales_lf.unique(subset=['store', 'dept', 'date'], keep='first')

        logger.info("Clipping negative weekly sales to 0", dataset="sales")
        sales_lf = sales_lf.with_columns(pl.col('weekly_sales').clip(lower_bound=0.0))
        ...
        sales_lf = sales_lf.with_columns(pl.col('date').str.to_date("%d/%m/%Y", strict=False)).sort('date', descending=False)
