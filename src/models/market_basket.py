...
def compute_market_basket() -> None:
    ...
    config.ADVANCED_ANALYTICS_DIR.mkdir(parents=True, exist_ok=True)
    
    # 3. Cast back to Polars for lightning-fast CSV export
    pl.DataFrame(strong_rules).write_csv(config.MARKET_BASKET_RULES_PATH)
    
    logger.info("Market Basket rules generated successfully!", rules_extracted=len(strong_rules), path=str(config.MARKET_BASKET_RULES_PATH))
