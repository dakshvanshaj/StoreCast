...
from models.trainer import prepare_data
...
def export_shap_analytics():
    ...
    # 1. Initialize Remote DagsHub Hooks
    dagshub.init(repo_owner='dakshvanshaj', repo_name='StoreCast', mlflow=True)
    
    # 2. Fetch the Production Model directly from the centralized Registry
    model_uri = "models:/StoreCast_XGBoost@production"
    logger.info("Downloading @production Artifacts from DagsHub Registry...", uri=model_uri)
    ...
    _, _, X_val, _, _, _, _, _ = prepare_data()
    ...
    try:
        ...
        X_train, _, _, _, _, _, _, _ = prepare_data()
